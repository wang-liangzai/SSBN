...
def adj_dependcy_tree(args, argments, max_length):
    ...
    # return ze, DN, DSN
    return ze
#def get_sentence_len(args, sentence):
#    tokenizer = BertTokenizer.from_pretrained(args.init_vocab, do_lower_case=args.do_lower_case)
#    en_nlp = spacy.load("en_core_web_sm")
#    doc = en_nlp((sentence))  # doc表示当前的sentence

#    sen_encoding = [tokenizer.convert_tokens_to_ids('[CLS]')]
#    for token in doc:
#        token_encoding = tokenizer.encode(token, add_special_tokens=True)  # 查询bert的词汇表的tokenizer
#        sen_encoding += token_encoding
#    sen_encoding += [tokenizer.convert_tokens_to_ids('[SEP]')]
#    sentence_len = len(sen_encoding)
#    return sentence_len
...
class Instance(object):
    def __init__(self, sentence_pack, args):
        ...
        id = sentence_pack['id']
        aspect_num = 0
        sentence = sentence_pack['sentence']                                   #获取到sentence的值，此处修改代码
...
def convert_examples_to_features(args, train_instances, max_span_length=8):
    ...
    for ex_index, example in enumerate(train_instances):
        ...
        sample['sentence'] = example.text_a                     #此处是sentences
        ...
        differ_opinion_sentiment = False              #置为False的目的？

        for triple_name in sample['triples']:
            aspect_span, opinion_span, sentiment = tuple(sample['triples'][triple_name][0]), tuple(
                sample['triples'][triple_name][1]), sample['triples'][triple_name][2]                    #分别将方面、意见、情感加载进入变量
            num_triple += 1                                                                              #三元组数量+1
            ...
            if opinion_span not in opinion_reverse:
                ...
                if opinion_reverse[opinion_span] != sentiment:
                    differ_opinion_sentiment = True                          #不同的意见词拥有不同的极性
                else:
                    aspect_reverse[opinion_span].append((aspect_span, sentiment))            #不同的意见词拥有相同的极性
        if differ_opinion_sentiment:                                                         #如果是不同的意见词情感极性
        ...
        if args.order_input:
            for i in range(max_span_length):
                ...
                for j in range(sample['text_length'] - i):
                    ...
                    if (j, i + j) not in opinion_reverse:                                  #找到相应的opinion span
                        reverse_opinion_label.append(0)
                    else:
                        # reverse_opinion_label.append(sentiment2id[opinion_reverse[(j, i + j)]])
                        reverse_opinion_label.append(validity2id[opinion_reverse[(j, i + j)]])          #validity2id none 0、积极1、消极1、中性1
...
#        sentences.append(sample['sentence'])
    return features, num_aspect, num_opinion                #此处返回值修改代码


def load_data(args, path, if_train=False):          #此处用json的方式尝试去读取文件，参数、路径
     #sentence_packs = json.load(open(path))         #找到path路径
     #if if_train:
     #    random.seed(args.RANDOM_SEED)              #设置随机数种子
     #    random.shuffle(sentence_packs)             #打乱顺序
     #instances = load_data_instances(sentence_packs, args)    #载入数据
     #tokenizer = BertTokenizer.from_pretrained(args.init_vocab, do_lower_case=args.do_lower_case)    #分词
     #data_instances, aspect_num, num_opinion, sentences = convert_examples_to_features(args, train_instances=instances,
     #                                                                       max_span_length=args.max_span_length)         #将数据变为特征,将句子返回为一个列表
     #list_instance_batch = []
     #list_sentences_batch = []                               #此处为修改的代码
     #for i in range(0, len(data_instances), args.train_batch_size):                      #此处修改数据大小
     #    list_instance_batch.append(data_instances[i:i + args.train_batch_size])
     #    list_sentences_batch.append(sentences[i:i + args.train_batch_size])
     #return list_instance_batch, list_sentences_batch
...
def load_data_instances_txt(lines):                                                        #此处是用line数据转化的格式
...
class DataTterator(object):
    def __init__(self, instances, args):
        self.instances = instances                              #里面包含sentence
        self.args = args
    ...
    def get_batch(self, batch_num):
        ...
        for i, sample in enumerate(self.instances[batch_num]):
            ...
            sentence_batch.append(sample['sentence'])
        ...
        return final_tokens_tensor, final_attention_mask, final_bert_spans_tensor, final_spans_mask_tensor, \
               ...
               final_related_spans_tensor, sentence_length, sentence_batch
    ...
    def get_input_tensors(self, tokenizer, tokens, spans, spans_ner_label, spans_aspect_label, spans_opinion_label,
        ...
        # 充在bert分出subword之后  需要对原有的aspect span进行补
