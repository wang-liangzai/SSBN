...
def stage_2_features_generation(bert_feature, attention_mask, spans, span_mask, spans_embedding, spans_aspect_tensor,
    ...
    all_span_mask = None
    spans_aspect_tensor_spilt = torch.chunk(spans_aspect_tensor, spans_aspect_tensor.shape[0], dim=0)  #将24*3的spans-aspect-tensor切分成24块
    for i, spans_aspect_tensor_unspilt in enumerate(spans_aspect_tensor_spilt):       #取每个切分好的方面跨度
        test = spans_aspect_tensor_unspilt.squeeze(0)
        batch_num = spans_aspect_tensor_unspilt.squeeze(0)[0]                    #第n个句子的标号
        ...
        span_index_start = torch.where(spans[batch_num, :, 0] == spans_aspect_tensor_unspilt.squeeze()[1],
                                       spans[batch_num, :, 1], torch.tensor(-1).type_as(spans))                   #用来找到跨度开始、结束位置的索引
        span_index_end = torch.where(span_index_start == spans_aspect_tensor_unspilt.squeeze()[2], span_index_start,
                                     torch.tensor(-1).type_as(spans))          #用已经预测的结束位置的标记来判断唯一的结束索引
        span_index = torch.nonzero((span_index_end > -1), as_tuple=False).squeeze(0)                   #找到跨度的索引值
        if min(span_index.shape) == 0:
            continue
        if spans_opinion_tensor is not None:              #传过来的数值是None
            spans_opinion_tensor_unspilt = spans_opinion_tensor[i,:].unsqueeze(0)   #
        aspect_span_embedding_unspilt = spans_embedding[batch_num, span_index, :].unsqueeze(0)   #将索引到的跨度转换为嵌入
        bert_feature_unspilt = bert_feature[batch_num, :, :].unsqueeze(0)         #取值后扩充为三维 第n个句子*100*768
        attention_mask_unspilt = attention_mask[batch_num, :].unsqueeze(0)     #掩码，将超过句子实际长度的那部分掩码掉
        spans_embedding_unspilt = spans_embedding[batch_num, :, :].unsqueeze(0)   #取句子的跨度嵌入
        span_mask_unspilt = span_mask[batch_num, :].unsqueeze(0)    #做跨度掩码
        if all_span_aspect_tensor is None:
            if spans_opinion_tensor is not None:
                all_span_opinion_tensor = spans_opinion_tensor_unspilt
            all_span_aspect_tensor = aspect_span_embedding_unspilt          #跨度的嵌入
            all_bert_embedding = bert_feature_unspilt       #该句中Bert的特征
            all_attention_mask = attention_mask_unspilt     #？用于特征掩码
            all_spans_embedding = spans_embedding_unspilt       #将跨度转换为特征
            ...
            all_span_mask = torch.cat((all_span_mask, span_mask_unspilt), dim=0)            #当没有意见tensor输入时，就没有相应的意见tensor输出
...
class Dim_Four_Block(torch.nn.Module):
    ...
    def forward(self, hidden_embedding, masks, encoder_embedding):
        ...
        masks = (~masks) * -1e9        #加1，取负
        attention_masks = masks[:, :, None, None, :]    #在None处增加维度
...
class Pointer_Block(torch.nn.Module):
    ...
    def forward(self, hidden_embedding, masks, encoder_embedding):
        ...
        #对应的score的维度为 (batch_size, num_heads, hidden_dim, encoder_dim)
        masks = (~masks) * -1e9
        if masks.dim() == 3:          #扩充为四维
            attention_masks = masks[:, None, :, :]
        elif masks.dim() == 2:        #扩充为四维
            if self.mask_for_encoder:           #取决于传进来的参数
...
def Loss(gold_aspect_label, pred_aspect_label, gold_opinion_label, pred_opinion_label, spans_mask_tensor, opinion_span_mask_tensor,
    ...
    gold_aspect_effective_label = torch.where(aspect_spans_mask_tensor, gold_aspect_label.view(-1),  #根据掩码的值，将未掩码的部分取值为标签值，掩码部分取值为-100
                                              torch.tensor(loss_function.ignore_index).type_as(gold_aspect_label))
    aspect_loss = loss_function(pred_aspect_label_logits, gold_aspect_effective_label)  #使用交叉熵计算方面损失
    ...
    # Loss反向
    reverse_opinion_span_mask_tensor = spans_mask_tensor.view(-1) == 1     #将mask全部转换为一维
    reverse_pred_opinion_label_logits = reverse_pred_opinion_label.view(-1, reverse_pred_opinion_label.shape[-1])   #将预测的标签转化为mask长度84
    reverse_gold_opinion_effective_label = torch.where(reverse_opinion_span_mask_tensor, reverse_gold_opinion_label.view(-1),
                                              torch.tensor(loss_function.ignore_index).type_as(reverse_gold_opinion_label))       #根据where函数，将真实标签获取到，掩码的部分用-100
    reverse_opinion_loss = loss_function(reverse_pred_opinion_label_logits, reverse_gold_opinion_effective_label)  #将真实值与预测值做loss
...
def shape_span_embedding(args, p, q, pad_mask, span_mask):  #此处的p和q是一样的
    kl_loss = 0
    input_size = p.size()
    assert input_size == q.size()
    for i in range(input_size[0]):
        span_mask_index = torch.nonzero(span_mask[i, :]).squeeze()   #span_mask为16*212；nonzero生成的数据是二维数组，其中啊有多少维度就有多少个一维数组；有多少个1就生成多少个一维数组
        lucky_squence = random.choice(span_mask_index)   #随机挑选一个跨度掩码索引
        P = p[i, lucky_squence, :]
        mask_index = torch.nonzero(pad_mask[i, lucky_squence, :])
        q_tensor = None
        for idx in mask_index:
            if idx == lucky_squence:    #如何相同的话，直接返回到for循环，不会执行下一个if
                continue
            if q_tensor is None:
                q_tensor = p[i, idx]     # 找到768维度的特征
            else:
