...
class Metric():
    def __init__(self, args, forward_pred_result, reverse_pred_result, gold_instances):
        ...
        self.pred_aspect = forward_pred_result[0]       # 预测的方面
        self.pred_aspect_sentiment = forward_pred_result[1]    # 预测的方面情感
        self.pred_aspect_sentiment_logit = forward_pred_result[2]    # 预测的方面情感概率

        self.pred_opinion = forward_pred_result[3]    # 预测的意见情感
        self.pred_opinion_sentiment_logit = forward_pred_result[4]    # 预测的意见情感概率

        '''反向评价'''
        self.reverse_pred_opinon = reverse_pred_result[0]    # 反向预测的意见
        self.reverse_pred_opinon_sentiment = reverse_pred_result[1]    # 反向预测的意见情感
        self.reverse_pred_opinon_sentiment_logit = reverse_pred_result[2]    # 反向预测的意见情感概率

        self.reverse_pred_aspect = reverse_pred_result[3]    # 反向预测的方面
        self.reverse_pred_aspect_sentiment_logit = reverse_pred_result[4]    # 反向预测的方面概率


    def P_R_F1(self, gold_num, pred_num, correct_num):    # 用于计算 P、R、F1
    ...
    def num_4_eval(self, gold, pred, gold_num, pred_num, correct_num):
        correct = set(gold) & set(pred)    # 预测的等于标签时为
    ...
    def cal_triplet_final_result(self, forward_results, forward_spans, reverse_results, reverse_spans):
        ...
        pred_dicts = {}   # 这里传值的三联体是已经形成三元组的三联体
        pred_spans = forward_spans + reverse_spans
        for index, result in enumerate(forward_results + reverse_results):    # 在正向、反向三元组中
            if result in pred_dicts:
                ...
                pred_dicts[result] = pred_spans[index]
        history = []
        for i in pred_dicts:    # 分两次循环所有预测的数值
            aspect_span_i = range(pred_dicts[i][0][0], pred_dicts[i][0][1])    # 找到方面的跨度标识
            opinion_span_i = range(pred_dicts[i][1][0], pred_dicts[i][1][1])    # 找到意见的跨度标识
            for j in pred_dicts:    # 第二次循环0
                if (i,j) in history:    # 两个key如果在记录中，返回循环
                    continue
                history.append((i, j))    # 将两个key添加到记录
                history.append((j, i))
                if i == j:    # 此次循环的是两个相同的key
                    continue
                aspect_span_j = range(pred_dicts[j][0][0], pred_dicts[j][0][1])    # 记录j的方面跨度
                opinion_span_j = range(pred_dicts[j][1][0], pred_dicts[j][1][1])    # 记录j的意见跨度
                repeat_a_span = list(set(aspect_span_i) & set(aspect_span_j))    # 此处是第一个for和第二和for一样的取值
                repeat_o_span = list(set(opinion_span_i) & set(opinion_span_j))    # 意见也是一样的取值
    ...
    def score_triples(self):
        ...
        for i in range(len(self.gold_instances)):
            ...
            bert_tokens = []
            spans = self.gold_instances[i]['spans']    # 得到所有的跨度
            start2idx = []
            end2idx = []
            ...
            pred_aspect_1 = pred_aspect
            reverse_aspect_1 = reverse_aspect
            pred_opinion_1 = pred_opinion
            reverse_opinion_1 = reverse_opinion
            ...
            if self.args.output_path:
                ...
                pred_text.append({"sentence": self.gold_instances[i]['sentence'],
                                  'gold_aspect': [gold_a for gold_a in set(gold_aspect)],
                                  'forward_aspect': [forward_a for forward_a in set(pred_aspect_1)],
                                  'reverse_aspect': [reverse_a for reverse_a in set(reverse_aspect_1)],
                                  'pred_aspect': [pred_a for pred_a in set(pred_aspect)],
                                  'gold_opinion': [gold_o for gold_o in set(gold_opinion)],
                                  'forward_opinion': [forward_o for forward_o in set(pred_opinion_1)],
                                  'reverse_opniion': [reverse_o for reverse_o in set(reverse_opinion_1)],
                                  'pred_opinion': [pred_o for pred_o in set(pred_opinion)],
                                  })
                for aspect in pred_aspect:
                    for gold in set(gold_aspect):
                        if aspect == gold:
                ...
                for opinion in pred_opinion:
                    for gold in set(gold_opinion):
                        if opinion == gold:
                            if len(opinion.split()) == 1:
                                opinion_1 += 1
                            elif len(opinion.split()) == 2:
                                opinion_2 += 1
        ...
        print('aspect_1', aspect_1)
        print('aspect_2', aspect_2)
        print('opinion_1', opinion_1)
        print('opinion_2', opinion_2)
        if self.args.output_path:
            F = open(self.args.dataset + 'triples.json', 'a', encoding='utf-8')
            json.dump(result, F, ensure_ascii=False, indent=4)
            F.close()

            F1 = open(self.args.dataset + 'aspect.json', 'a', encoding='utf-8')
            json.dump(aspect_text, F1, ensure_ascii=False, indent=4)
            F1.close()

            F2 = open(self.args.dataset + 'opinion.json', 'a', encoding='utf-8')
            json.dump(opinion_text, F2, ensure_ascii=False, indent=4)
            F2.close()

            F3 = open(self.args.dataset + 'comparison.json', 'a', encoding='utf-8')
            json.dump(pred_text, F3, ensure_ascii=False, indent=4)
            F3.close()
    ...
    def find_aspect_sentiment(self, sentence_index, bert_spans, span, aspect_sentiment, aspect_sentiment_logit):
        # span = [span[1], span[2], ]
        bert_span_index = [i for i,x in enumerate(bert_spans) if span[1] == x[0] and span[2] == x[1]]    # 找到答案的预测值的位置
    ...
    def find_gold_triples(self, sentence_index, bert_spans, bert_tokens):
        ...
        triples = self.gold_instances[sentence_index]['triples']    # 找到这一个句子里面的所有三元组
        for keys in triples:    # key的形式为 方面词|意见词
            ...
            for aspect_token in aspect.split( ):
                ...
                if self.args.span_generation == "CNN":
                    ...
                    aspect_tokens += token
            new_aspect = self.gold_token(aspect_tokens)    # 找到真实的方面术语
            ...
            new_opinion = self.gold_token(opinion_tokens)    # 找到真实的意见术语

            sentiment = triples[keys][2]    # 找到情感标签

            triples_list.append((new_aspect, new_opinion, sentiment.lower()))    # 组成一个标签三元组

            aspect_list.append((new_aspect))    # 将方面添加到方面列表
            opinion_list.append((new_opinion))    # 将意见添加到意见列表

            apce_list.append((new_aspect, sentiment))    # 将方面、情感添加到apce
            pair_list.append((new_aspect, new_opinion))    # 将方面、意见添加到对
    ...
    def find_pred_triples(self, sentence_index, bert_spans, bert_tokens):
        ...
        for i, pred_aspect in enumerate(pred_aspect_span):
            if len(new_aspect_span) == 0:
                ...
                if pred_aspect[1] == new_aspect_span[-1][1]:
                    ...
                    new_aspect_span.append(pred_aspect)    # 将预测方面加入到方面跨度
        for j, pred_aspect in enumerate(new_aspect_span):
            ...
            for opinion_span in new_opinion_spans:
                ...
                opinion = self.find_token(bert_tokens, opinion_span)    # 从句子中找到意见词
