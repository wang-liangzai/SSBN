...
from models.data_BIO_loader import load_data, DataTterator, adj_dependcy_tree
...
def eval(gcn_model, bert_model, step_1_model, step_2_forward, step_2_reverse, dataset, args):
    ...
    step_1_model.zero_grad()
    step_2_reverse.zero_grad()
    step_2_forward.zero_grad()
    ...
    with torch.no_grad():
        ...
        for j in tqdm.trange(dataset.batch_count):
            if j == 1:
                start = time.time()
            sentence_adj = []
            ...
            reverse_aspect_label_tensor, related_spans_tensor, sentence_length, sentence_batch = dataset.get_batch(j)
            ...
            for sentence in sentence_batch:
               sentence_adj.append(adj_dependcy_tree(args, sentence, 100))
            bert_features = bert_model(input_ids=tokens_tensor, attention_mask=attention_mask)            #此处获取到了bert_features#
            sentence_tensor = torch.FloatTensor(np.array(sentence_adj))

            h_gcn, pool_mask = gcn_model(sentence_tensor.to(args.device), bert_features.last_hidden_state)
            bert_features.last_hidden_state = bert_features.last_hidden_state + h_gcn
            if j == 0 and args.model_para_test:
                step_1_model.to("cpu")
...
def train(args):
    ...
    print('开始加载测试集')
    logger.info('开始加载测试集')
    test_datasets = load_data(args, test_path, if_train=False)      #此处修改代码
    ...
    if args.mode == 'train':
        ...
        print('开始加载训练与验证集')
        train_datasets = load_data(args, train_path, if_train=False)          #此处修改代码
        trainset = DataTterator(train_datasets, args)
        print("Train features build completed")
        print("Dev features build beginning")
        dev_datasets = load_data(args, dev_path, if_train=False)              #此处修改代码
        ...
        tot_loss = []
        tot_kl_loss = []
        best_aspect_f1, best_opinion_f1, best_APCE_f1, best_pairs_f1, best_triple_f1, avg_triple_f1= 0,0,0,0,0,0
 ...
 #            for j in range(trainset.batch_count):
                ...
                tokens_tensor, attention_mask, bert_spans_tensor, spans_mask_tensor, spans_ner_label_tensor, \
                spans_aspect_tensor, spans_opinion_label_tensor, reverse_ner_label_tensor, reverse_opinion_tensor, \
                reverse_aspect_label_tensor, related_spans_tensor, sentence_length, sentence_batch = trainset.get_batch(j)


                for sentence in sentence_batch:
                    sentence_adj.append(adj_dependcy_tree(args, sentence, 100))                 #表示成什么形式？


                bert_output = Bert(input_ids=tokens_tensor, attention_mask=attention_mask)
                sentence_tensor = torch.FloatTensor(np.array(sentence_adj))

                h_gcn, pool_mask = gcn(sentence_tensor.to(args.device), bert_output[0])
                bert_output.last_hidden_state = bert_output.last_hidden_state + h_gcn

                aspect_class_logits, opinion_class_logits, spans_embedding, forward_embedding, reverse_embedding, \
                cnn_spans_mask_tensor = step_1_model(bert_output.last_hidden_state,
                                                     attention_mask,
                                                     bert_spans_tensor,
                                                     spans_mask_tensor,
                ...
                tot_kl_loss.append(kl_loss)
            logger.info(('Loss:', sum(tot_loss)/ len(tot_loss)))
            logger.info(('KL_Loss:', sum(tot_kl_loss)/len(tot_kl_loss)))
            tot_loss = []
            tot_kl_loss = []
            ...
            #                                                                                step2_reverse_model,
            #                                                                                devset, args)
            aspect_result, opinion_result, apce_result, pair_result, triplet_result = eval(gcn, Bert, step_1_model,
                                                                                           step2_forward_model,
                                                                                           step2_reverse_model,
                                                                                           devset, args)
            print('Evaluating complete')
            #avg_triple_f1 = avg_triple_f1 + triplet_result[2]
            #j = i + 1
            #print('avg_triple_f1: ', avg_triple_f1/j)

            ################################################此处开始单独测试
#            if triplet_result[2] > 0.40:
#
#                logger.info("Evaluation on testset:")
#                logger.info("单轮测试开始----------------------------")
#                model_path = args.model_dir + args.dataset + '_' + str(i) + '_' + str(aspect_result[2]) + '.pt'
#                state = {
...
#                gcn.load_state_dict(state['gcn_model'])
...
#            if triplet_result[2] > best_triple_f1 and triplet_result[2] > 0.55:
            if triplet_result[2] > 0.55:
                model_path = args.model_dir + args.dataset + '_' + str(i) + '_' + str(triplet_result[2]) + '.pt'
                state = {
                    "gcn_model": gcn.state_dict(),
                    "bert_model": Bert.state_dict(),
                    "step_1_model": step_1_model.state_dict(),
    ...
    model_path = args.model_dir + args.dataset + '_' + str(0.7692307692307692) + '.pt'
    if args.muti_gpu:
        state = torch.load(model_path)
    else:
        print('*'*50)
...
def main():
    ...
    parser.add_argument("--dataset_path", default="./datasets/ASTE-Data-V2-EMNLP2020/",
                        ...
                        help="")
    parser.add_argument("--dataset", default="res16", type=str, choices=["lap14", "res14", "res15", "res16"],
