...
def eval(gcn_model, bert_model, step_1_model, step_2_forward, step_2_reverse, dataset, args):
    ...
    with torch.no_grad():
        ...
        for j in range(dataset.batch_count):
            ...
            '''双方向预测'''
            if torch.nonzero(pred_aspect_logits, as_tuple=False).shape[0] == 0:              #非0元素的索引值  .shape=23*2
                ...
                for pred_aspect_span in pred_aspect_spans:
                    ...
                    span_aspect_tensor_unspilt = torch.tensor(
                        (batch_num, span_aspect_tensor_unspilt_1[0], span_aspect_tensor_unspilt_1[1])).unsqueeze(0)   #找到方面跨度的，句子、开始位置、结束位置
                ...
                _, all_span_aspect_tensor, all_bert_embedding, all_attention_mask, all_spans_embedding, all_span_mask = stage_2_features_generation(
                    bert_features.last_hidden_state, attention_mask, bert_spans_tensor, spans_mask_tensor,
                    forward_embedding, pred_span_aspect_tensor)              #每一个batcch都得出结果
        ...
        reverse_pred_data = (reverse_stage1_pred_opinion_result, reverse_stage1_pred_opinion_with_sentiment,
                             reverse_stage1_pred_opinion_sentiment_logit, reverse_stage2_pred_aspect_result,
                             reverse_stage2_pred_aspect_sentiment_logit)
        reverse_pred_result = unbatch_data(reverse_pred_data)       #这里得到的数据分为五个部分，1第一阶段预测的值；2，第一阶段预测的情感；3第一阶段预测情感的概率；4，第二阶段预测的值；5，第二阶段预测情感的概率
...
def train(args):
    ...
    #model_path = args.model_dir + args.dataset + '_' + str(0.6530612244897959) + '.pt'
    if args.muti_gpu:
...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=80, help='training epoch number')
